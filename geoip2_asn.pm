...
sub Init_geoip2_asn {
    ...
    my ($datafile,$override)=split(/\+/,$InitParams,2);
   	if (! $datafile) { $datafile="GeoLite2-ASN.mmdb"; }
	...
	$geoip2_asn = GeoIP2::Database::Reader->new(
        file    => $datafile,
        locales => [ 'en', 'de', ]);
	$LoadedOverride=0;
	if ($geoip2_asn) { debug(" Plugin $PluginName: GeoIP2 plugin and gi object initialized",1); }
...
#-----------------------------------------------------------------------------
sub AddHTMLMenuLink_geoip2_asn {
    my $categ=$_[0];
    my $menu=$_[1];
    my $menulink=$_[2];
    my $menutext=$_[3];
    ...
    if ($categ eq 'who') {
        ...
        $menutext->{"plugin_$PluginName"}='ASN'; # Text
    }
	# ----->
	return 0;
}
...
sub AddHTMLGraph_geoip2_asn {
	...
	my $title="GeoIP2 ASN";
	&tab_head($title,19,100,'asn');
	print "<tr bgcolor=\"#$color_TableBGRowTitle\">";
	print "<th>ASN: ".((scalar keys %_asn_h)-($_asn_h{'unknown'}?1:0))."</th>";
	...
	my $count=0;
	&BuildKeyList($MaxRowsInHTMLOutput,$MinHit{'Cities'},\%_asn_h,\%_asn_h);
    	foreach my $key (@keylist) {
            if ($key eq 'unknown') { next; }
   		    my ($asn)=split('_', $key, 2);
            $asn=~s/%20/ /g;
   			my $p_p; my $p_h;
			if ($TotalPages) { $p_p=int(($_asn_p{$key}||0)/$TotalPages*1000)/10; }
   			if ($TotalHits)  { $p_h=int($_asn_h{$key}/$TotalHits*1000)/10; }
   		    print "<tr>";
   		    print "<td class=\"aws\">".ucfirst(EncodeToPageCode($asn))."</td>";
    		if ($ShowCities =~ /P/i) { print "<td>".($_asn_p{$key}?Format_Number($_asn_p{$key}):"&nbsp;")."</td>"; }
    		if ($ShowCities =~ /P/i) { print "<td>".($_asn_p{$key}?"$p_p %":'&nbsp;')."</td>"; }
    		if ($ShowCities =~ /H/i) { print "<td>".($_asn_h{$key}?Format_Number($_asn_h{$key}):"&nbsp;")."</td>"; }
    		if ($ShowCities =~ /H/i) { print "<td>".($_asn_h{$key}?"$p_h %":'&nbsp;')."</td>"; }
    		if ($ShowCities =~ /B/i) { print "<td>".Format_Bytes($_asn_k{$key})."</td>"; }
    		if ($ShowCities =~ /L/i) { print "<td>".($_asn_p{$key}?Format_Date($_asn_l{$key},1):'-')."</td>"; }
    		print "</tr>\n";
...
sub ShowInfoHost_geoip2_asn {
	...
	if ($param eq '__title__')
	{
		...
		print "<th width=\"80\">";
        print "<a href=\"".($ENV{'GATEWAY_INTERFACE'} || !$StaticLinks?XMLEncode("$AWScript?${NewLinkParams}output=plugin_$PluginName"):"$StaticLinks.plugin_$PluginName.$StaticExt")."\"$NewLinkTarget>GeoIP2<br/>ASN</a>";
        print "</th>";
	}
	elsif ($param)
	{
		my ($country, $city, $subdivision) = Lookup_geoip2_asn($param);
		print "<td>";
...
# UNIQUE: NO (Several plugins using this function can be loaded)
#-----------------------------------------------------------------------------
sub SectionInitHashArray_geoip2_asn {
	# <-----
...
# UNIQUE: NO (Several plugins using this function can be loaded)
#-----------------------------------------------------------------------------
sub SectionProcessIp_geoip2_asn {
	my $param = shift;
	my $rec = 'unknown';
	my ($country, $city, $subdivision) = Lookup_geoip2_asn($param);
	if ($country && $city) {
...
# UNIQUE: NO (Several plugins using this function can be loaded)
#-----------------------------------------------------------------------------
sub SectionProcessHostname_geoip2_asn {
	return SectionProcessIp_geoip2_asn(@_);
}
...
# UNIQUE: NO (Several plugins using this function can be loaded)
#-----------------------------------------------------------------------------
sub SectionReadHistory_geoip2_asn {
    my $issectiontoload=shift;
    my $xmlold=shift;
    my $xmleb=shift;
	my $countlines=shift;
	# <-----
	if ($Debug) { debug(" Plugin $PluginName: Begin of PLUGIN_geoip2_asn section"); }
	my @field=();
...
# UNIQUE: NO (Several plugins using this function can be loaded)
#-----------------------------------------------------------------------------
sub SectionWriteHistory_geoip2_asn {
	...
	foreach (@keylist) {
		$keysinkeylist{$_}=1;
		print HISTORYTMP "${xmlrb}".XMLEncodeForHisto($_)."${xmlrs}0${xmlrs}", $_asn_h{$_}, "${xmlrs}0${xmlrs}0${xmlre}\n"; next;
	}
	foreach (keys %_asn_h) {
...
#-----------------------------------------------------------------------------
sub LoadOverrideFile_geoip2_asn{
	my $filetoload="";
	if ($OverrideFile){
...
# GEOIP entry
#-----------------------------------------------------------------------------
sub Lookup_geoip2_asn {
	$param = shift;
	if (!$LoadedOverride) { &LoadOverrideFile_geoip2_asn(); }
	if ($Debug) { debug("  Plugin $PluginName: Lookup_geoip2_asn for $param",5); }
	if ($geoip2_asn && !exists($TmpDomainLookup{$param})) {
		...
		for (@result) {
			...
			if ($Debug && $param ne $address) { debug("  Plugin $PluginName: Lookup_geoip2_asn $param resolved to $address",5); }
			eval {
				my $record = $geoip2_asn->asn(ip => $address);
				# FIXME
				...
				my $asn = $record->autonomous_system_organization();
				$TmpDomainLookup{$param} = [ $asn, $asn ];
				last;
			}
		}
	}
	my @res = @{ $TmpDomainLookup{$param} };
	if ($Debug) { debug("  Plugin $PluginName: Lookup_geoip2_asn for $param: [@res]",5); }
	return @res;
}
